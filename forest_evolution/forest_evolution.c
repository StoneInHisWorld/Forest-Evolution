...
void free_stage_UI(int round);
void prey(Player *players, int damage);
void evolve_stage(int round);
void big_round(Player *players, int *damage);
void random_players(Player *players);
...
int main()
{
	...
	while (choice != 1 && choice != 2)
	{
		...
		if (choice == 1)
		{
			// erase enter
			getchar();
			players = (Player *)malloc(NUM_PLAYER * sizeof(Player));
...
void regular_round(Player *players, int damage)
{
	free_stage_UI(damage - 1);
	srand((unsigned)time(NULL));
	printf("第%d轮捕食阶段，从玩家%d号开始，", damage - 1, rand() % 10 + 1);
	prey(players, damage);
	evolve_stage(damage - 1);
	return;
}

void free_stage_UI(int round)
{
	printf("第%d轮自由阶段，%d分钟计时开始（按任意键结束）\n", round, FREESTAGE_SPAN);
	getchar();
	// 可以用血量或卡牌交换血量或者卡牌
}
...
void big_round(Player *players, int *damage)
{
	...
	//getchar();
	evolve_stage(*damage / 2 + 1);
}

//void regular_round(Player *players, int damage) 
//{
//	printf("第%d轮自由阶段，%d分钟计时开始（按任意键结束）\n", damage - 1, FREESTAGE_SPAN);
//	getchar();
//	srand((unsigned)time(NULL));
//	printf("第%d轮捕食阶段，从玩家%d号开始，", damage - 1, rand() % 10 + 1);
//	int p1, p2;
...
void random_players(Player *players)
{
	//char id_preset[10][2] = { {JOKER, Joker}, {SPADE, K},
	//{HEART, K}, {CLUB, K}, {SPADE, Q}, {HEART, Q}, {CLUB, Q},
	//{SPADE, J}, {HEART, J}, {CLUB, J} };
	Player preset_players[10] = { {JOKER, Joker}, {SPADE, K},
	{HEART, K}, {CLUB, K}, {SPADE, Q}, {HEART, Q}, {CLUB, Q},
	{SPADE, J}, {HEART, J}, {CLUB, J} };
	srand((unsigned)time(NULL));
	printf("随机生成玩家只支持10位\n");
	...
	{
		// 随机选择一个身份赋予i+1号玩家
		int rand_index = rand() % (9 - i + 1);
		players[i] = preset_players[rand_index];
		// 与最后一位预设玩家调换，之后不再进行选择
		preset_players[rand_index] = preset_players[9 - i];
		preset_players[9 - i] = players[i];
	}
	return;
}
